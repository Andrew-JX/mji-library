...
<template>
  <div class="json-lab">
    ...
    <section class="lab-section">
      ...
      <p>We operate in: {{ bookstores.topSellers.join(', ') }}</p>
      <p>Our #1 seller: {{ bookstores.countries.join(', ') }}</p>
    </section>
    ...
    <section class="lab-section">
      ...
      <!-- Show this only when showMessage 为 true -->
    ...
    <section class="lab-section">
      ...
      <ul>
        <li v-for="author in authors" :key="author.id" :class="{ highlight: author.name === 'George Orwell' }"
          :style="author.name === 'George Orwell' ? 'color: red; font-weight: bold' : ''">
          {{ author.name }}
        </li>
      </ul>
    </section>
...
import bookstoresData from '../assets/json/bookstores.json'
// Activity 1: Import JSON files (authors.json and bookstores.json)
// TODO: CODE TO IMPORT JSON FILES HERE
const authors = ref(authorsData)
const bookstores = ref(bookstoresData)
...
const orwell = computed(() =>
  ...
  authors.value.find(author => author.name === 'George Orwell')
)

// Activity 5: Find author by ID
const austen = computed(() =>
  // TODO: CODE TO FIND AUTHOR BY ID HERE
  authors.value.find(author => author.id === 1)
)
</script>
